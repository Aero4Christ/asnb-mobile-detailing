...
const serviceAreas = ["Henrietta", "Vernon", "Wichita Falls", "Burke Burnett", "Jacksboro", "Lakeside City", "Holiday", "Petrolia", "Iowa Park"];
    ...
    <main className="min-h-screen overflow-hidden bg-[#020b22] pb-20 text-white md:pb-0">
      ...
      <section id="top" className="hero-bay relative isolate min-h-[775px] overflow-hidden">
        ...
        <div className="relative mx-auto flex min-h-[775px] max-w-[1440px] flex-col justify-end px-5 pb-14 pt-20 md:px-9 lg:px-14 lg:pb-16">
          <div className="ml-auto max-w-[720px] text-right"><p className="eyebrow text-[#f7cf58]">Mobile Detailing & Mobile Oil Changes</p><h1 className="mt-5 font-display text-[clamp(4.5rem,11vw,10.5rem)] leading-[0.77] uppercase tracking-[-0.045em] text-white">From Dirt to <span className="gold-text">New.</span><br />We Come To <span className="gold-text">You.</span></h1><p className="mt-7 ml-auto max-w-xl text-lg leading-8 text-white/82">Premium results at your home, work, or wherever your vehicle is parked. A’s n B brings the care, equipment, and finishing touch to you.</p><div className="mt-9 flex flex-wrap justify-end gap-3"><a href={phoneHref} className="gold-button"><Phone className="size-4" /> Call to book</a><a href={`mailto:${email}?subject=Appointment%20Request%20for%20A%27s%20n%20B%20Mobile%20Detailing`} className="blue-button"><Mail className="size-4" /> Email us</a></div></div>
          <div className="mt-14 flex flex-wrap justify-end gap-x-7 gap-y-3 border-t border-white/20 pt-5 text-[11px] font-semibold uppercase tracking-[0.13em] text-white/70"><span className="flex items-center gap-2"><BadgeCheck className="size-3.5 text-[#f7cf58]" /> Professional Detailing</span><span className="flex items-center gap-2"><ShieldCheck className="size-3.5 text-[#f7cf58]" /> Licensed & insured</span><span className="flex items-center gap-2"><BadgeCheck className="size-3.5 text-[#f7cf58]" /> Clean, protect, restore</span></div>
        </div>
      </section>

      <section className="brand-navy relative px-5 py-16 md:px-9 md:py-20 lg:px-14"><div className="gold-rule mx-auto mb-12 max-w-[1310px]" /><div className="mx-auto grid max-w-[1310px] gap-10 lg:grid-cols-[0.77fr_1.23fr] lg:gap-24"><div><p className="eyebrow text-[#f7cf58]">THE A’S N B PROMISE</p><h2 className="mt-5 font-display text-5xl leading-[0.84] uppercase tracking-[-0.04em] sm:text-7xl">Quality you can see.<br /><span className="text-[#0875ed]">Service you can trust.</span></h2></div><div className="max-w-2xl"><p className="text-xl leading-8 text-white/88">Professional vehicle care should fit your life. That is why A’s n B is built around premium mobile service, clear starting prices, and the kind of finish you are proud to drive.</p><p className="mt-5 max-w-xl leading-7 text-white/60">From a thorough interior refresh to exterior protection and mobile oil changes, every appointment is focused on your vehicle’s condition and what will make the biggest difference.</p><div className="mt-8 flex items-center gap-3 border-l-2 border-[#e95591] pl-4 text-sm text-white/80"><Heart className="size-4 fill-[#e95591] text-[#e95591]" /> That’s our promise.</div></div></div></section>

      <section id="services" className="blue-panel relative px-5 py-20 md:px-9 md:py-28 lg:px-14"><GoldSpark className="absolute right-[7%] top-16 size-6" /><div className="mx-auto max-w-[1310px]"><div className="grid gap-8 border-b border-[#f7cf58]/45 pb-9 lg:grid-cols-[1fr_0.75fr] lg:items-end"><div><p className="eyebrow text-[#f7cf58]">WHAT WE DO</p><h2 className="mt-5 font-display text-5xl leading-[0.84] uppercase tracking-[-0.04em] text-white sm:text-7xl">Mobile service.<br /><span className="text-[#f7cf58]">Serious shine.</span></h2></div><p className="max-w-lg leading-7 text-white/75">Interior, exterior, protection, restoration, and on-the-go maintenance. Choose the service that fits your vehicle, then call or text to get your appointment on the calendar.</p></div><div className="mt-10"><div className="service-carousel" aria-label="Our services carousel">{detailServices.map((service, index) => <article key={service} className="service-slide"><span className="service-slide-number">0{index + 1}</span><span className="service-slide-title">{service}</span></article>)}</div><p className="mt-3 text-center text-[10px] font-bold uppercase tracking-[0.16em] text-white/45">Swipe to explore services 1 through 8</p></div><div className="mt-12 flex flex-wrap gap-4"><a className="gold-button" href={phoneHref}>Book your appointment <ArrowUpRight className="size-4" /></a><a className="text-link" href={`mailto:${email}`}>Ask about a custom service <ArrowDownRight className="size-4" /></a></div></div></section>
      ...
      <section id="locations" className="brand-navy relative px-5 py-20 md:px-9 md:py-28 lg:px-14"><div className="mx-auto grid max-w-[1310px] gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-16"><div className="gold-frame overflow-hidden"><img src="/assets/Wichita-Falls-map.png" alt="Map of Wichita Falls, Texas and surrounding service areas" className="aspect-[1.18] w-full object-cover" /></div><div><p className="eyebrow text-[#f7cf58]">SERVICE LOCATION</p><h2 className="mt-5 font-display text-5xl leading-[0.84] uppercase tracking-[-0.04em] text-white sm:text-7xl">Wichita Falls,<br /><span className="text-[#0875ed]">Texas.</span></h2><p className="mt-6 max-w-xl text-lg leading-8 text-white/75">A’s n B Mobile Detailing comes to you throughout Wichita Falls and the surrounding communities.</p><div className="mt-8 grid grid-cols-2 gap-x-6 gap-y-3 border-t border-white/15 pt-6 text-sm font-semibold uppercase tracking-[0.08em] text-white/80 sm:grid-cols-3">{serviceAreas.map((area) => <span key={area} className="flex items-center gap-2"><MapPin className="size-3.5 text-[#f7cf58]" /> {area}</span>)}</div><a href={phoneHref} className="gold-button mt-9">Check your service area <Phone className="size-4" /></a></div></div></section>
      ...
      <footer className="bg-black px-5 pb-8 pt-12 md:px-9 lg:px-14"><div className="mx-auto grid max-w-[1310px] gap-10 border-b border-[#d99a17]/35 pb-10 md:grid-cols-[1fr_auto] md:items-end"><div className="flex items-center gap-4"><BrandLogo className="size-20 rounded-full object-cover" /><div><p className="font-roman text-lg font-semibold uppercase tracking-[0.12em] text-[#f7cf58]">A’s n B</p><p className="mt-1 text-xs uppercase tracking-[0.17em] text-white/70">Mobile Detailing</p><p className="mt-3 text-sm text-white/60">Quality You Can See. Service You Can Trust.</p><p className="mt-2 text-sm text-white/60">Wichita Falls, Texas.</p></div></div><div className="text-left text-sm leading-7 text-white/70 md:text-right"><a className="block transition hover:text-[#f7cf58]" href={phoneHref}>{phoneText}</a><a className="block transition hover:text-[#f7cf58]" href={`mailto:${email}`}>{email}</a></div></div><div className="mx-auto flex max-w-[1310px] flex-col gap-3 pt-6 text-[10px] font-medium uppercase tracking-[0.12em] text-white/45 md:flex-row md:justify-between"><p>© {new Date().getFullYear()} A’s n B Mobile Detailing</p><p>Professional detailing. Premium results.</p></div></footer>

      <div className="mobile-cta-bar fixed inset-x-0 bottom-0 z-50 flex gap-3 border-t border-[#0875ed] bg-[#0754c7] p-3 shadow-[0_-8px_24px_rgba(0,0,0,0.35)] md:hidden"><a href={phoneHref} className="mobile-cta-button"><Phone className="size-4" /> Call Now</a><a href={`mailto:${email}?subject=Appointment%20Request%20for%20A%27s%20n%20B%20Mobile%20Detailing`} className="mobile-cta-button"><Mail className="size-4" /> Email Us</a></div>
